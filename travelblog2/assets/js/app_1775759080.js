...
function renderTripDetail(t) {
  ...
  const tlItems = posts.map((p, i) => {
    ...
    const tipBadges = (p.tips||[]).map(t => `<span class="badge badge-${t.type}" style="margin-right:3px">${t.title}</span>`).join('');
    const card = `<div class="tl-card" onclick="openPost(${JSON.stringify(p).replace(/"/g,'&quot;')})">
      ...
      <div class="tl-card-body">
        ...
        ${tipBadges ? `<div style="margin-top:5px">${tipBadges}</div>` : ''}
      </div>
    </div>`;
...
function openLightbox(src, type) {
  ...
  lb.innerHTML = `<span class="lightbox-close" onclick="document.getElementById('lightbox').classList.remove('open')">×</span>
    ${type==='video' ? `<video controls autoplay src="${src}"></video>` : `<img src="${src}" alt="">`}`;
  lb.classList.add('open');
  lb.onclick = e => { if(e.target===lb) lb.classList.remove('open'); };
}

function renderMediaGrid(mediaArr, editable) {
  if (!mediaArr || !mediaArr.length) return '';
  return `<div class="post-media-grid">${mediaArr.map(m => `
    <div class="post-media-item">
      ${m.type === 'video'
        ? `<video controls src="${m.path}"></video>`
        : `<img src="${m.path}" onclick="openLightbox('${m.path}','image')" alt="">`}
      ${editable ? `<button class="post-media-remove" onclick="deletePostMedia(${m.id},this)">✕</button>` : ''}
    </div>`).join('')}</div>`;
}

function buildCarousel(allMedia, isAdmin) {
  if (!allMedia || !allMedia.length) return '';
  const main = allMedia[0];
  const thumbs = allMedia.slice(1);
  const mainHtml = main.type === 'video'
    ? `<video controls src="${main.path}" style="width:100%;max-height:480px;object-fit:cover;border-radius:12px;display:block"></video>`
    : `<img src="${main.path}" id="carousel-main-img" onclick="openLightbox('${main.path}','image')" style="width:100%;max-height:480px;object-fit:cover;border-radius:12px;cursor:pointer;display:block" alt="">`;
  const thumbsHtml = thumbs.length ? `<div style="display:flex;gap:8px;margin-top:8px;flex-wrap:wrap">
    ${thumbs.map((m,i) => m.type==='video'
      ? `<video src="${m.path}" style="width:100px;height:70px;object-fit:cover;border-radius:8px;cursor:pointer" onclick="openLightbox('${m.path}','video')"></video>`
      : `<img src="${m.path}" style="width:100px;height:70px;object-fit:cover;border-radius:8px;cursor:pointer;border:2px solid transparent" onclick="swapCarouselMain('${m.path}',this)" alt="">`
    ).join('')}
  </div>` : '';
  const removeBtn = isAdmin ? `<button class="post-media-remove" onclick="deletePostMedia(${main.id},this.closest('.carousel-wrap'))">✕</button>` : '';
  return `<div class="carousel-wrap" style="position:relative;margin-bottom:1rem">${removeBtn}${mainHtml}${thumbsHtml}</div>`;
}

function swapCarouselMain(newSrc, thumbEl) {
  const mainImg = document.getElementById('carousel-main-img');
  if (mainImg) {
    mainImg.src = newSrc;
    mainImg.onclick = () => openLightbox(newSrc, 'image');
  }
  document.querySelectorAll('.carousel-wrap img[style*="width:100px"]').forEach(el => el.style.border = '2px solid transparent');
  thumbEl.style.border = '2px solid var(--c-accent)';
}
...
function openPost(p) {
  ...
  // Combine legacy image with media array
  let allMedia = [];
  if (p.image) allMedia.push({id: null, path: p.image, type: 'image'});
  if (p.media && p.media.length) allMedia = allMedia.concat(p.media);

  const carouselHtml = buildCarousel(allMedia, isAdmin);

  const postTips = (p.tips||[]).length ? `
    <div style="margin-top:1.5rem">
      <div style="font-size:13px;font-weight:600;color:var(--c-text2);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.75rem">Tipps & Tricks</div>
      ${p.tips.map(t => `<div class="tip-card" style="margin-bottom:.5rem">
        <div class="tip-card-body">
  ...
  d.innerHTML = `
    <button class="btn btn-sm" onclick="document.getElementById('post-detail').classList.remove('open');document.getElementById('post-detail').innerHTML=''" style="margin-bottom:1rem">← Schliessen</button>
    <h2 style="font-size:22px;font-weight:700;margin-bottom:1rem">${p.title}</h2>
    ${carouselHtml}
    <div class="post-content" style="margin-top:${carouselHtml?'1rem':'0'}">${p.content||''}</div>
    ${postTips}
    ${isAdmin ? `<div style="margin-top:1.5rem;display:flex;gap:8px;flex-wrap:wrap">
      <button class="btn btn-sm btn-primary" onclick="editPost(${JSON.stringify(p).replace(/"/g,'&quot;')})">Bearbeiten</button>
      <button class="btn btn-sm" onclick="openMediaUpload(${p.id})">+ Medien</button>
      <button class="btn btn-sm" onclick="openPostTipEditor(${p.id})">+ Tipp</button>
      <button class="btn btn-sm btn-danger" onclick="deletePost(${p.id})">Löschen</button>
    </div>` : ''}
  `;
...
async function deletePostMedia(id, btn) {
  await api('posts','delete_media',{id});
  btn.closest('.post-media-item').remove();
}

function openMediaUpload(postId) {
  document.getElementById('mu-post-id').value = postId;
  document.getElementById('mu-files').value = '';
  document.getElementById('mu-progress').textContent = '';
  document.getElementById('media-upload-modal').classList.add('open');
}

async function uploadPostMedia() {
  const postId = document.getElementById('mu-post-id').value;
  const files = document.getElementById('mu-files').files;
  if (!files.length) return;
  const prog = document.getElementById('mu-progress');
  for (let i = 0; i < files.length; i++) {
    prog.textContent = `Lade hoch ${i+1}/${files.length}...`;
    const fd = new FormData();
    fd.append('file', files[i]);
    fd.append('post_id', postId);
    await api('posts','upload_media',null,fd);
  }
  prog.textContent = 'Fertig!';
  setTimeout(() => { closeMediaUpload(); if (state.currentTrip) openTrip(state.currentTrip.id); }, 800);
  showToast('Medien hochgeladen!');
}

function closeMediaUpload() { document.getElementById('media-upload-modal').classList.remove('open'); }

async function openPostTipEditor(postId) {
  // Ensure trips are loaded for the dropdown
  if (!state.trips.length) await loadTrips();
  openTipEditor({}, postId);
}
...
function editPost(p) {
  ...
  // Show existing media
  const preview = document.getElementById('pe-existing-media');
  const allMedia = [];
  if (p.image) allMedia.push({id:null,path:p.image,type:'image'});
  if (p.media) allMedia.forEach ? null : null;
  if (p.media && p.media.length) p.media.forEach(m => allMedia.push(m));
  if (allMedia.length) {
    preview.innerHTML = '<div style="font-size:12px;color:var(--c-text2);margin-bottom:6px">Vorhandene Medien</div><div style="display:flex;gap:6px;flex-wrap:wrap">'
      + allMedia.map(m => `<div style="position:relative">
          ${m.type==='video'
            ? `<video src="${m.path}" style="width:80px;height:60px;object-fit:cover;border-radius:6px"></video>`
            : `<img src="${m.path}" style="width:80px;height:60px;object-fit:cover;border-radius:6px">`}
          ${m.id ? `<button onclick="deletePostMedia(${m.id},this.parentElement)" style="position:absolute;top:2px;right:2px;background:rgba(0,0,0,.6);color:white;border:none;border-radius:3px;font-size:10px;padding:1px 4px;cursor:pointer">✕</button>` : ''}
        </div>`).join('')
      + '</div>';
...
async function savePostEditor() {
  ...
  if (d.ok) {
    const files = document.getElementById('pe-imgs').files;
    for (let i = 0; i < files.length; i++) {
      const fd = new FormData();
      fd.append('file', files[i]);
      fd.append('post_id', d.id);
      await api('posts','upload_media',null,fd);
    }
    closePostEditor();
  ...
  document.getElementById('pe-save-btn').disabled = false;
}
...
/* ---- TIP EDITOR ---- */
async function openTipEditor(tip, linkPostId) {
  const t = tip || {};
  ...
  document.getElementById('tipe-link-post-id').value = linkPostId || '';
  document.getElementById('tipe-save-btn').disabled = false;
  // Populate trip dropdown
  const tripSel = document.getElementById('tipe-trip-id');
  if (tripSel) {
    const trips = state.trips.length ? state.trips : (await fetch('api/trips.php?action=list').then(r=>r.json()).then(d=>d.trips||[]));
    tripSel.innerHTML = '<option value="">— Keine —</option>'
      + trips.map(tr => `<option value="${tr.id}">${tr.title}</option>`).join('');
    if (state.currentTrip) tripSel.value = state.currentTrip.id;
    if (t.trip_ids) {
      const firstId = t.trip_ids.split(',')[0];
      if (firstId) tripSel.value = firstId;
    }
  }
...
async function saveTipEditor() {
  const saveBtn = document.getElementById('tipe-save-btn');
  if (saveBtn.disabled) return;
  saveBtn.disabled = true;
  const id = document.getElementById('tipe-id').value;
  const tripId = document.getElementById('tipe-trip-id').value;
  const linkPostId = document.getElementById('tipe-link-post-id').value;
  const body = {
    id: id || null,
  ...
  if (!body.title) { saveBtn.disabled = false; return; }
  const d = await api('tips', 'save', body);
  if (d.ok) {
    if (linkPostId) await api('posts','link_tip',{post_id: parseInt(linkPostId), tip_id: d.id});
    closeTipEditor();
