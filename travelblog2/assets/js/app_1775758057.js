...
function openPostTipEditor(postId) {
  document.getElementById('pte-post-id').value = postId;
  document.getElementById('pte-title').value = '';
  document.getElementById('pte-desc').value = '';
  document.getElementById('pte-content').value = '';
  document.getElementById('pte-type').value = 'tip';
  document.getElementById('post-tip-editor-modal').classList.add('open');
}

async function savePostTip() {
  const postId = document.getElementById('pte-post-id').value;
  const body = {
    title: document.getElementById('pte-title').value,
    description: document.getElementById('pte-desc').value,
    content: document.getElementById('pte-content').value,
    type: document.getElementById('pte-type').value,
    published: 1,
    trip_ids: state.currentTrip ? [state.currentTrip.id] : []
  };
  if (!body.title) return;
  const d = await api('tips','save',body);
  if (d.ok) {
    await api('posts','link_tip',{post_id: parseInt(postId), tip_id: d.id});
    document.getElementById('post-tip-editor-modal').classList.remove('open');
    if (state.currentTrip) openTrip(state.currentTrip.id);
    showToast('Tipp gespeichert!');
  }
}

function closePostTipEditor() { document.getElementById('post-tip-editor-modal').classList.remove('open'); }
